...
import { CheckmarkCircle01Icon, Image01Icon, CircleArrowDataTransferHorizontalIcon } from 'hugeicons-react'
...
export function FileItem({ state, name, size, type }: FileItemProps) {
  ...
  return (
    <div className={base()}>
      ...
      {state === 'complete' ? (
        <CheckmarkCircle01Icon className="h-5 w-5 fill-violet-600 text-white dark:fill-violet-300 dark:text-zinc-900" />
      ) : (
        <Button type="button" variant="ghost" className={deleteButton()}>
          <CircleArrowDataTransferHorizontalIcon className="h-5 w-5" />
        </Button>
      )}
    </div>
  )
